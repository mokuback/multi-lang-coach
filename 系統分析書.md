...
作為整個應用的 Entry Point，負責管理所有的全域狀態（API Key、Target Language、User Context 等），並作為 Props 傳遞給子元件。同時負責渲染 Settings (設定介面) 與 Welcome Modal，並於設定頁面顯示由 Vite 注入之版號 (`__APP_VERSION__`) 與建置日期 (`__BUILD_DATE__`)。
...
1. **Init**: Create Vite React app. Build `index.css` with CSS variables. Modify `vite.config.js` to define global variables `__APP_VERSION__` and `__BUILD_DATE__`. Configure `eslint.config.js` globals accordingly.
