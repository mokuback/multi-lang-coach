...
const LANGUAGE_MAP: Record<string, LanguageInfo> = {
  en: {
    ...
    uiLabel: 'English',
    phoneticFormat: '該單字的KK音標，若為長句可為空字串',
    partOfSpeechFormat: '該單字的詞性簡寫(如 n., vi., adj.)，若非單一字彙則可為空字串',
    termLabel: {
  ...
  ja: {
    ...
    uiLabel: '日本語',
    phoneticFormat: '該單字的平假名拼音，並在後方括號內標註重音數字(例如：きのう [0])。如果單字本身為純假名，請只保留重音數字(例如：[1])',
    partOfSpeechFormat: '該單字的日文詞性簡寫(如 名, 動, 形, 副)，若非單一字彙則可為空字串',
    termLabel: {
  ...
  'zh-CN': {
    ...
    uiLabel: '简体中文',
    phoneticFormat: '該漢字的拼音，並在後方括號內標註聲調數字(例如：māo [1])。若為詞組，每個字的拼音用空格隔開',
    partOfSpeechFormat: '該單字的詞性簡寫(如 n., v., adj.)，若非單一字彙則可為空字串',
    termLabel: {
  ...
  ko: {
    ...
    uiLabel: '한국어',
    phoneticFormat: '該韓文的拼音(羅馬字轉寫)，若為長句可為空字串',
    partOfSpeechFormat: '該單字的韓文詞性簡寫(如 명사, 동사, 형용사)，若非單一字彙則可為空字串',
    termLabel: {
...
export function getPhoneticFormat(langId: string): string {
  return LANGUAGE_MAP[langId]?.phoneticFormat || '該單字的音標';
}

/** 取得詞性格式指令 */
export function getPartOfSpeechFormat(langId: string): string {
  return LANGUAGE_MAP[langId]?.partOfSpeechFormat || '該單字的詞性簡寫';
}
