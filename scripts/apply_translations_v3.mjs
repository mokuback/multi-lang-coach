...
// Process: for each word, find its block and replace
// The file has this format for en section:
// { word: 'Hello', meanings: {\n      'zh-TW': '你好',...\n    }, phonetic: '...' }

let replaceCount = 0;

for (const [word, translations] of Object.entries(dict)) {
  // Create the old block pattern (all languages have the same Chinese translation)
  // We need to match the actual Chinese value in the file
  // For "Hello", the zh-TW value is '你好', so all fields are '你好'
  ...
  // Find the pattern: word: '...', meanings: { ... }
  // Use a regex that matches across lines
  const regex = new RegExp(
    `word: '${word.replace(/'/g, "\\'")}',\\s*meanings: \\{[^}]+\\}`,
    'g'
  );
  
  // Actually, this regex won't match across lines. Let me use a different approach.
  // Let me find the index of `word: '${word}'` and then find the meanings block after it.
  
  const wordIndex = content.indexOf(`word: '${word}'`, content.indexOf('  en: ['));
  if (wordIndex === -1) {
  ...
  const meaningsStart = content.indexOf('meanings: {', wordIndex);
  if (meaningsStart === -1) {
  ...
  const meaningsEnd = i;
  const oldMeaningsBlock = content.substring(meaningsStart, meaningsEnd);
  
  // Replace
  content = content.substring(0, meaningsStart) + newMeanings + content.substring(meaningsEnd);
  replaceCount++;
}

console.log(`Replaced ${replaceCount} vocabulary items`);
