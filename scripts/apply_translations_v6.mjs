...
// Find the en: section range
const enStart = content.indexOf('  en: [');
// Find the end of en: section (closing ] at the correct depth)
let bracketCount = 0;
let enEnd = enStart;
...
for (const [word, translations] of Object.entries(dict)) {
  let wordIndex = content.indexOf(`word: '${word}'`, enStart);
  while (wordIndex !== -1 && wordIndex < enEnd) {
    // Find the meanings block
    const meaningsStart = content.indexOf('meanings: {', wordIndex);
    if (meaningsStart === -1 || meaningsStart > enEnd) break;
    ...
    // Build new meanings with escaped values
    ...
    replacements.push({ start: meaningsStart, end: meaningsEnd, newText: newMeanings });
    console.log(`Found: ${word} at ${wordIndex}`);
    
    // Find next occurrence
    wordIndex = content.indexOf(`word: '${word}'`, wordIndex + 1);
  }
}

console.log(`Found ${replacements.length} items to replace`);
...
console.log(`Applied ${replacements.length} replacements. File updated.`);
