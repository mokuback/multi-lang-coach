...
// Translation dictionary for all 75 English words
const dict = {
...
console.log(`Translation dictionary: ${Object.keys(dict).length} words`);

// Process the file: find each word in en section and replace its meanings
// Approach: use regex to match word + meanings block across multiple lines
const wordPattern = /word: '([^']+)',\s*\n\s*meanings: \{([^}]+)\}/gs;
...
while ((match = wordPattern.exec(content)) !== null) {
  const word = match[1];
  const translations = dict[word];
  
  if (!translations) {
    console.log(`Warning: No translation for "${word}"`);
    continue;
  }
  
  // Build the new meanings string
  const newMeanings = `word: '${word}',\n        meanings: {
  ...
  // Replace in updatedContent
  const oldBlock = match[0];
  updatedContent = updatedContent.replace(oldBlock, newMeanings);
  replaceCount++;
}
