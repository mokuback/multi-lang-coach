...
// Replace meanings in the en section string
let newEnSection = enSection;
let replaceCount = 0;

for (const [word, translations] of Object.entries(dict)) {
  // Create the old pattern (all languages have the same value, which is the zh-TW translation)
  // The old pattern is: meanings: { 'zh-TW': 'X', 'zh-CN': 'X', 'en': 'X', ... }
  // where X is the Chinese translation of the English word
  
  // Actually, let's just find and replace the meanings block for each word
  // Use regex to match word + meanings block
  const regex = new RegExp(
    `word: '${word.replace(/'/g, "\\'")}',\\s*\\n\\s*meanings: \\{[^}]+\\}`,
    'g'
  );
  
  const matches = [...newEnSection.matchAll(regex)];
  for (const match of matches) {
    const oldBlock = match[0];
    
    // Build new meanings block
    const newMeanings = `word: '${escape(word)}',\n        meanings: {
    ...
    newEnSection = newEnSection.replace(oldBlock, newMeanings);
    replaceCount++;
...
content = content.substring(0, enStart) + newEnSection + content.substring(enEnd);
