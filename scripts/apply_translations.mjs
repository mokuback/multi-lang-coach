...
// Complete translation dictionary for all 75 English vocab words
// Format: 'English Word': { 'zh-TW': '', 'zh-CN': '', 'ja': '', 'ko': '', 'es': '', 'fr': '', 'de': '' }
const translations = {
  ...
  'Will': { 'zh-TW': '將', 'zh-CN': '将', 'ja': 'だろう', 'ko': '할 것이다', 'es': 'Will', 'fr': 'Volonté', 'de': 'Wille' },
  ...
  'Than': { 'zh-TW': '比', 'zh-CN': '比', 'ja': 'より', 'ko': '보다', 'es': 'Que', 'fr': 'Que', 'de': 'Als' },
...
console.log(`Translation dictionary created with ${Object.keys(translations).length} words`);

// Read the curriculumData.ts file
let content = fs.readFileSync('src/data/curriculumData.ts', 'utf8');
...
// For each word in the translation dictionary, replace its meanings in the en section
let replacements = 0;
for (const [word, meanings] of Object.entries(translations)) {
  // Create the meanings object string
  const meaningsStr = `meanings: {
      'zh-TW': '${meanings['zh-TW']}',
      'zh-CN': '${meanings['zh-CN']}',
      'en': '${word}',
      'ja': '${meanings['ja']}',
      'ko': '${meanings['ko']}',
      'es': '${meanings['es']}',
  ...
  // Find and replace the meanings for this word in the en section
  const wordPattern = new RegExp(`word: '${word.replace(/'/g, "\\'")}',\\s*meanings: \\{[^}]+\\}`, 'g');
  const newWordPattern = `word: '${word}',\n        ${meaningsStr}`;
  
  // Actually, let's do a more targeted replacement
  // Find the line with `word: '${word}'` and replace the following meanings block
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lines[i].includes(`word: '${word}'`)) {
      // Found the word, now replace the meanings block
      newLines.push(lines[i]);
      newLines.push('        ' + meaningsStr);
      // Skip the old meanings block (assuming it's 10 lines)
      skipLines = 10;
      replacements++;
      continue;
    }
  ...
  if (replacements > 0) {
    enSection = newLines.join('\n');
  }
}

console.log(`Replaced ${replacements} vocabulary items`);

// Write the updated content back
const newContent = content.substring(0, enStart) + enSection + content.substring(enEnd);
fs.writeFileSync('src/data/curriculumData.ts', newContent, 'utf8');

console.log('Updated curriculumData.ts');
