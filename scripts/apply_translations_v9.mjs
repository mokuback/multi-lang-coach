/**
 * Translate curriculumData.ts en: section meanings
 * Uses tsx to properly import and modify the TypeScript file
 */

import { writeFileSync } from 'fs';

// We need to import the data, but we can't directly import .ts file from .js script
// Instead, let's read the file as string and do careful replacement

const filePath = 'src/data/curriculumData.ts';
let content = await import('fs').then(fs => fs.readFileSync(filePath, 'utf8'));
...
if (enStart === -1) {
  console.error('Error: en: section not found');
  process.exit(1);
}

// Find matching ] for en: [
let bracketCount = 0;
let enEnd = enStart;
...
// Process replacements one by one, from END to START to preserve indices
const replacements = [];

for (const [word, translations] of Object.entries(dict)) {
  let wordIndex = enSection.indexOf(`word: '${word}'`);
  while (wordIndex !== -1) {
    // Find meanings block
    const meaningsStart = enSection.indexOf('meanings: {', wordIndex);
    if (meaningsStart === -1) break;
    ...
    wordIndex = enSection.indexOf(`word: '${word}'`, wordIndex + 1);
  }
}
...
for (const rep of replacements) {
  enSection = enSection.substring(0, rep.start) + rep.text + enSection.substring(rep.end);
}
